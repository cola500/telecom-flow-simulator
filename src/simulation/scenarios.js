...
const PRODUCTS = {
  fiber: {
    ...
    resources: [
      { name: "RO: Fiber access", desc: "Tillgänglig fiberport på OLT + fiberpar fram till adressen", pattern: "fiber_access" },
      { name: "RO: Network profile", desc: "VLAN, QoS, IP ur pool, tjänsteprofil", pattern: "network_profile" },
      { name: "RO: CPE / Router", desc: "Hårdvara hos kund + initial konfig", pattern: "cpe_router" },
...
const PATTERNS = {
  ...
  customer_order: {
    group: "concepts",
    title: "Vad är en Customer Order?",
  ...
  resource_order: {
    group: "concepts",
    title: "Vad är en Resource Order?",
  ...
  decomposition_why: {
    group: "concepts",
    title: "Varför ordernedbrytning är viktig",
  ...
  provisioning_activation: {
    group: "automation",
    title: "Provisioning, activation och automationens gränser",
    body: "*Provisioning* är att skicka konfiguration till nätet — NETCONF/CLI mot routers, OLT, IMS-core, SBC. *Activation* är att verifiera att tjänsten faktiskt fungerar end-to-end. Det är två olika saker, även om de ofta hanteras av samma system och ibland slås ihop i språkbruket. Skillnaden spelar roll: en config kan accepteras av elementen men tjänsten ändå inte fungera (CPE inte konfigurerad rätt, IP routing mismatch, port aktiverad fel) — det är *partial activation*, en av de dyraste felmoderna eftersom kunden tror att tjänsten är klar. Reservation (Resource Inventory) är *ytterligare* en separat sak: att låsa rätt port/IP/profile *innan* aktiveringen börjar. Reservation utan aktivering ger 'orphaned resources'; aktivering utan reservation ger race conditions där två orders konkurrerar om samma resurs.\n\n*Automation* av provisioning/activation är en klassisk modernisering i operatörer — från manuella ärenden hos NOC, via skript-baserad provisioning, till intent-based aktivering med template-bibliotek och CI/CD för nätconfig. Vinsterna är reella: ledtid sjunker (från timmar/dagar till minuter), variation minskar (samma input → samma output), och team-tid frigörs från repetitivt arbete. Men automation tar inte bort alla fel. Den eliminerar inte:\n\n- *Inventory mismatch* — om templaten utgår från en port som inte finns där den ska, får du fortfarande ActivationRejected.\n- *Nätfel* — element down, fiber broken, EMS unresponsive — automation kan inte trolla fram en fungerande infrastruktur.\n- *Felaktiga produktregler* — en automatiserad mall som översätter customer order fel ger fortfarande fel tjänst, bara snabbare.\n- *Partial activation-risken* — automation kan trycka config snabbt, men om du inte automatiserat *verifieringen* har du bara automatiserat halva problemet.\n\nDärför ska *Billing trigger* aldrig kopplas till orderstatus eller config-acceptans — den ska kopplas till verifierad aktivering. En bra fråga att fundera på: är automatiseringen *intent-based* (man beskriver önskat sluttillstånd, systemet räknar ut config) eller *imperativ* (man skickar specifika kommandon)? Det första kräver mer mogen inventory men ger bättre rollback och mindre custom-templating per produkt."
  },
  fiber_access: {
    group: "product",
    title: "Vad är fiber access (port + fiberpar)?",
    body: "*Fiber access* är den fysiska anslutningspunkten — en port på en OLT (Optical Line Terminal) plus ett fiberpar som går från OLT:n fram till kundens adress. För att en fiber-order ska kunna levereras måste det finnas (1) en ledig port på rätt OLT, och (2) ett fiberpar som täcker fram till adressen där kunden bor.\n\nNyckelpoäng: porten och fibern är de mest *fysiskt begränsade* resurserna i hela flödet. Andra resurser (VLAN, IP, profil) kan typiskt allokeras ur stora logiska pooler — men det finns bara så många portar i en OLT, och fiber dras inte över natten. Det är därför fiber-rollout är så långsiktig och kapitalintensiv: man bygger ut kapaciteten innan kunderna efterfrågar den, eller så missar man försäljning.\n\n*Adresskvalitet* är central: om CRM:s adressformat inte exakt matchar fiber-nätets adresskatalog (gatunamn med stavning, lägenhetsnummer, fastighetsbeteckningar) kan en order avvisas i feasibility-check trots att fibern faktiskt finns där.\n\nVanlig fel-källa: feasibility säger 'ledig port' baserat på inventory, men porten är i verkligheten upptagen sedan en tidigare order som aldrig städats upp — *inventory drift*. Det syns inte förrän provisioning försöker konfigurera porten och får 'already in use'."
  },
  network_profile: {
    group: "product",
    title: "Vad är en network profile (VLAN, QoS, IP)?",
    body: "*Network profile* är konfigurationspaketet som binder kundens fiberport till en logisk bredbandstjänst. Profilen specificerar:\n\n- *VLAN* — vilket logiskt nätverk porten ska tillhöra (t.ex. konsumentnät vs B2B).\n- *QoS* — Quality of Service-policy: prioritet mellan trafiktyper, garanterad bandbredd, burst-budget.\n- *IP-konfiguration* — IP-adress (statisk ur reserverat block eller dynamisk från DHCP-pool), default gateway, routing-information.\n- *Tjänsteprofil* — överenskommen bandbredd (500/500 Mbps), abuse-policy, ev. fast IP-tjänst.\n\nProfilen styr *vad kunden faktiskt får* när access är på plats. Fel profil = fel hastighet eller fel IP-setup. Profilen är vanligen parametriserad i provisioning-systemet och appliceras via NETCONF (eller motsvarande protokoll) mot OLT och edge-router.\n\nNyckelpoäng: network profile är pedagogiskt en bra påminnelse om att *aktivering* är mer än bara att 'slå på porten'. Du kan ha en fungerande fysisk anslutning och fortfarande inte ge kunden tjänsten — om profilen är fel eller om någon annan resurs i kedjan inte är konfigurerad.\n\nVanlig fel-källa: 'service active' enligt systemen men kunden får ingen IP — profilen pushades, NETCONF svarade OK, men något i DHCP-poolen eller routingen är fel. Det är *partial activation* i miniatyr."
  },
  cpe_router: {
    group: "product",
    title: "Vad är CPE / Router?",
    body: "*CPE* (Customer Premises Equipment) är hårdvaran hos kunden — typiskt en router som kopplas till fiber-uttaget hemma. Operatören tillhandahåller och konfigurerar CPE:n så att den passar nätets profil för just den kunden.\n\nResource order för CPE täcker två steg:\n\n- *Allokering av enhet* — vilken CPE-modell, från lagret eller via installation. Olika modeller stödjer olika hastigheter, WiFi-standarder, eller B2B-funktioner.\n- *Initial konfiguration* — CPE:n behöver veta sin VLAN, sina credentials och sin tjänsteprofil. Konfiguration sker antingen *pre-staged* (innan utskick — CPE kommer färdig till kund) eller *zero-touch* (CPE laddar config från en provisioneringsserver vid första uppkoppling).\n\nNyckelpoäng: CPE är ofta där 'enkla' fel uppstår — fel template för en specifik CPE-modell, en adapter som inte svarar, eller en CPE som inte hittar sin config-server. Det är också där *kunden själv* har inflytande (strömkabel, placering, fysisk koppling) vilket gör felsökning bredare än 'bara nätproblem'.\n\nMognad-perspektiv: zero-touch provisioning är en av de tydligaste automationsvinsterna i en konsumentoperatör — eliminerar pre-staging-logistik och låter samma CPE-stock användas för olika tjänsteprofiler. Men det kräver mogen integration mellan CPE-modellen, ACS (Auto Configuration Server) och provisioneringssystem.\n\nVanlig fel-källa: kund får CPE:n men den 'lyser fel' — typiskt en config-mismatch som upptäcks först vid installationen, vilket genererar ett supportsamtal."
  },
  msisdn: {
    group: "product",
    title: "Vad är MSISDN?",
  ...
  sim_esim: {
    group: "product",
    title: "Vad är SIM/eSIM-provisionering?",
  ...
  fiber_vs_mobile: {
    group: "product",
    title: "Varför fiber och mobil dekomponeras olika",
  ...
  provisioning_capacity: {
    group: "automation",
    title: "Provisioning capacity vs automation",
    body: "Två oberoende sätt att förbättra provisioning/activation. De ser ofta lika ut i KPI:erna men är *helt olika* förbättringar.\n\n*Capacity* = hur många activation-jobb systemet kan hantera parallellt. I simulatorn motsvarar det 1–3 'workers' i Provisioning. I verkligheten:\n\n- Fler adapter-instanser mot nätelementen.\n- Parallellt kapacitet i orchestration/workflow-motorn.\n- Bättre köhantering med flera samtidiga aktiveringar.\n- 'Workers' är *inte* nödvändigtvis personer — det är en pedagogisk modell för parallell teknisk aktiveringskapacitet.\n\n*Automation* = hur mycket varje enskild aktivering är automatiserad. I simulatorn motsvarar det toggle 'Automated provisioning'. I verkligheten:\n\n- Skript- vs template- vs intent-baserad provisioning.\n- Automatiserade vs manuella valideringssteg.\n- Snabbare config-push och färre handpåläggningar.\n\nDe påverkar olika saker:\n\n- Capacity påverkar *parallellism* — hur många orders i flykt samtidigt.\n- Automation påverkar *duration och felrisk per order* — hur snabbt och pålitligt en enskild aktivering går.\n\nViktigt: ingendera av dem eliminerar fel. Höjd capacity flyttar bottleneck men löser inte inventory mismatch eller nätfel. Ökad automation snabbar upp men tar inte bort partial activation-risken om verifieringen inte också är automatiserad.\n\nOch oavsett vilket: *billing får inte triggas förrän activation är verifierad.* Det är en separat disciplin från både capacity och automation.\n\nExperiment att prova:\n\n- RI=3, Prov=1 → Provisioning blir bottleneck (för få parallella aktiveringar).\n- RI=1, Prov=3 → Resource Inventory är fortfarande bottleneck (RI:s 5s reservation dominerar).\n- Båda=3 + Automated provisioning på → kortare lead time, men incidents finns kvar."
  },
  common_across_products: {
    group: "product",
    title: "Vad är gemensamt mellan produkter?",
...
//
// Två produkt-specifika happy paths. Båda har samma "operating pattern":
//   Customer Order → Service Order → (Feasibility) → Reservation → Provisioning
//   → Activation → Verification → BillingStartRequested
// — men de tekniska stegen och deras antal skiljer sig per produkt.
//
// Varje step har ett *role*-fält som engine använder för automation och
// fail-prob-logik (oberoende av tech-namnet, som är produkt-specifikt och
// visas i log/timeline). System-IDs (crm/si/ri/prov/billing) är samma för
// båda produkter så bottleneck-logik och queue-panel fungerar utan ändring.

const HAPPY_PATH_FIBER = [
  { role: "order_created", tech: "CustomerOrderCreated", system: "crm", duration: 800,
  ...
  { role: "service_order", tech: "ServiceOrderCreated", system: "si", duration: 600,
    domain: "Service Inventory skapar tjänsteinstans 'Broadband Access' kopplad till kunden.",
    teach: "Lär dig: service order är teknisk men på hög abstraktionsnivå — kundagnostisk mall som parametriseras per order." },
  { role: "feasibility", tech: "FeasibilityChecked", system: "si", duration: 1500,
    domain: "Service Inventory verifierar att fiberkapacitet finns för adressen — service location-check.",
    teach: "Lär dig: feasibility skiljer 'kan vi sälja det' från 'kan vi leverera det'. För fiber är det adressens täckning." },
  { role: "reservation", tech: "ResourcesReserved", system: "ri", duration: 5000,
    domain: "Fiberport, network profile (VLAN/QoS/IP), CPE/router reserveras parallellt.",
    teach: "Lär dig: en service order genererar flera resource orders. Här uppstår fall-out om inventory drift finns." },
  { role: "provisioning", tech: "ProvisioningStarted", system: "prov", duration: 1500,
    domain: "Access-profil pushas till OLT via NETCONF.",
    teach: "Lär dig: provisioning är där OSS möter verkligheten — config skickas till nätelementen." },
  { role: "provisioning", tech: "CPEConfigurationStarted", system: "prov", duration: 1500,
    domain: "CPE/router konfigureras med kundspecifika parametrar (VLAN-tag, IP, autentisering).",
    teach: "Lär dig: CPE-config är ofta separat från access-config — två olika element med olika fail-modes." },
  { role: "activation", tech: "ServiceActivated", system: "prov", duration: 600,
    domain: "Tjänsten aktiveras i nätet — paketen kan börja flöda. Service Inventory uppdateras till 'active'.",
    teach: "Lär dig: aktivering ≠ verifierad leverans förrän vi testat end-to-end." },
  { role: "verification", tech: "ServiceVerified", system: "prov", duration: 600,
    domain: "End-to-end-test bekräftar att tjänsten faktiskt fungerar från CPE till bakhalsen.",
    teach: "Lär dig: utan post-activation verification riskerar vi 'partial activation' — tjänsten är 'aktiv' men fungerar inte." },
  { role: "billing", tech: "BillingStartRequested", system: "billing", duration: 800,
    domain: "Trigger skickas till charging-systemet — själva fakturan produceras nedströms.",
    teach: "Lär dig: detta är en signal, inte en faktura. Triggern bör synka mot verifierad aktivering — inte mot orderstatus." }
];

const HAPPY_PATH_MOBILE = [
  { role: "order_created", tech: "CustomerOrderCreated", system: "crm", duration: 800,
    domain: "Säljaren har lagt en kommersiell order. CRM och Order Management har en gemensam förståelse av kund + produkt.",
    teach: "Lär dig: ordern är kommersiell tills den dekomponeras till tekniska delar." },
  { role: "service_order", tech: "ServiceOrderCreated", system: "si", duration: 600,
    domain: "Service Inventory skapar tjänsteinstans 'Mobile Connectivity' (subscription + voice + data).",
    teach: "Lär dig: även mobile har en kundagnostisk service-mall, parametriserad per kund — t.ex. 50 GB + EU-roaming." },
  { role: "reservation", tech: "NumberReserved", system: "ri", duration: 2000,
    domain: "MSISDN allokeras ur nummerpool — eller portering från annan operatör startas.",
    teach: "Lär dig: MSISDN är en regulatorbegränsad resurs och portabel mellan operatörer (number portability)." },
  { role: "reservation", tech: "SimProfileReserved", system: "ri", duration: 3000,
    domain: "SIM/eSIM-profil kopplas till IMSI och bindas i HSS/UDM. eSIM kräver ofta extern RSP.",
    teach: "Lär dig: SIM/eSIM-provisionering involverar fler system än fiber-reservation — ny fail-mode (RSP timeout, profile mismatch)." },
  { role: "provisioning", tech: "SubscriberProfileProvisioningStarted", system: "prov", duration: 2500,
    domain: "Subscription profile (plan, QoS, datapaket, voice/SMS) pushas till HSS/UDM.",
    teach: "Lär dig: HSS/UDM är 'logisk-nätet' — inga fysiska element men kritisk databas-synk över flera system." },
  { role: "activation", tech: "NetworkSubscriptionActivated", system: "prov", duration: 1000,
    domain: "Nätbindning slutförs — telefonen kan nu hitta ett nät om den slås på.",
    teach: "Lär dig: aktivering i mobile = nätbindning. Inga fält-ingrepp, bara databas-uppdateringar — men de måste alla synka." },
  { role: "verification", tech: "RegistrationReadinessVerified", system: "prov", duration: 800,
    domain: "Verifiering att telefonen kan registrera sig och autentisera mot nätet.",
    teach: "Lär dig: även för mobile är post-activation verifiering kritiskt — annars kan kund få 'aktiverad' SIM som inte fungerar i fältet." },
  { role: "billing", tech: "BillingStartRequested", system: "billing", duration: 800,
    domain: "Trigger skickas till charging-systemet — själva fakturan produceras nedströms.",
    teach: "Lär dig: detta är en signal, inte en faktura. Triggern bör synka mot verifierad aktivering — inte mot orderstatus." }
];

// Bakåtkompatibilitet: HAPPY_PATH alias för fiber (default-produkt)
const HAPPY_PATH = HAPPY_PATH_FIBER;

function happyPathFor(productId) {
  return productId === "mobile" ? HAPPY_PATH_MOBILE : HAPPY_PATH_FIBER;
}

// Fail-flöden bygger på alla steg fram tills failure-rolen, plus själva
// fail-eventet. Det ger ett logiskt fail-flöde per produkt utan att duplicera
// step-data.
function failResourceFor(productId) {
  const path = happyPathFor(productId);
  const before = [];
  for (const step of path) {
    if (step.role === "reservation") break;
    before.push(step);
  }
  return [...before, {
    role: "fail",
    tech: "ResourceUnavailable",
    system: "ri",
    fail: true,
    duration: 4000,
    domain: "Resource Inventory hittar ingen ledig resurs för ordern — flödet stoppas. Vanligaste underliggande orsak är inventory drift, inte att resursen faktiskt saknas i nätet.",
...
function failProvFor(productId) {
  const path = happyPathFor(productId);
  const before = [];
  for (const step of path) {
    if (step.role === "activation") break;
    before.push(step);
  }
  return [...before, {
    role: "fail",
    tech: "ActivationRejected",
    system: "prov",
    fail: true,
    duration: 3000,
